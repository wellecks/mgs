...
def train(model, tokenizer, dataset_tensor_dict, args, device):
    ...
    for epoch_number in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # -- MLE case
            if torch.rand(1).item() < args.mrt_mle_mix:
                ...
                loss, batch_metrics = mrt_utils.mrt_loss(
                    model, batch, candidates, distances, tokenizer.pad_token_id, tokenizer.eos_token_id,
                    args.mrt_num_samples,
                    args.mrt_normalize_distance,
                    args.mrt_offset_zero
                )
...
def add_args(parser):
    parser.add_argument("--mrt-mle-mix", type=float, default=0.0)
    parser.add_argument("--mrt-alpha", type=float, default=1.0)
    parser.add_argument("--mrt-num-samples", type=int, default=4)
    parser.add_argument("--mrt-normalize-distance", type=int, default=1, choices=[0, 1])
    parser.add_argument("--mrt-offset-zero", type=int, default=0, choices=[0, 1])
    parser.add_argument(
        "--mrt-include-target", type=int, choices=[0, 1], default=0
    )
    parser.add_argument(
        "--mrt-include-greedy", type=int, choices=[0, 1], default=0
    )
    return parser